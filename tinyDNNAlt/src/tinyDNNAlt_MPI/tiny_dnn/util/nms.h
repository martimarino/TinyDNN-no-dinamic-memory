...
inline float_t iou(const bounding_box &b1, const bounding_box &b2) {
  ...
  float_t width        = std::max<float_t>(float_t(0.0f), (x2 - x1 + float_t(1)));
  float_t height       = std::max<float_t>(float_t(0.0f), (y2 - y1 + float_t(1)));
  float_t intersection = width * height;
  float_t _iou         = intersection / (b1.area() + b2.area() - intersection);
  return _iou >= float_t(0) ? _iou : float_t(0);
}
