  ...
  void back_propagation(const std::vector<tensor_t *> &in_data,
    ...
    for_i(prev_delta.size(), [&](size_t sample) {
      ...
      for (size_t i = 0; i < sz; ++i) {
        prev_delta[sample][i] = float_t(double(mask_[sample][i])) * curr_delta[sample][i];
      }
    });
  }
    ...
    for_i(sample_count, [&](size_t sample) {
      ...
      if (phase_ == net_phase::train) {
        for (size_t i = 0; i < in_vec.size(); i++)
          mask[i]     = bernoulli(double(dropout_rate_));

        for (size_t i = 0; i < in_vec.size(); i++)
          out_vec[i]  = float_t(double(mask[i])) * scale_ * in_vec[i];
      } else {
