...
class quantized_deconvolutional_layer : public layer {
 ...
 private:
  ...
  static size_t deconv_out_length(size_t in_length,
                                  size_t window_size,
                                  size_t stride) {
    return (size_t)ceil((double)(in_length)*stride + window_size - 1);
  }
  ...
  static size_t deconv_out_unpadded_length(size_t in_length,
    ...
    return pad_type == padding::same
             ? (size_t)ceil((double)in_length * stride)
             : (size_t)ceil((double)(in_length)*stride + window_size - 1);
  }
  ...
  void copy_and_unpad_output(const tensor_t &out) {
    ...
    dws.curr_out_buf_ =
      tensor_t(out.size(), vec_t(params_.out_unpadded.size(), float_t(0)));
