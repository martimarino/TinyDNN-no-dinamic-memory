...
class batch_normalization_layer : public layer {
 public:
  ...
  batch_normalization_layer(const layer &prev_layer,
                            float_t epsilon  = float_t(1e-5),
                            float_t momentum = float_t(0.999),
  ...
  void post_update() override {
    for (size_t i = 0; i < mean_.size(); i++) {
      mean_[i] = momentum_ * mean_[i] + (float_t(1) - momentum_) * mean_current_[i];
      variance_[i] =
        momentum_ * variance_[i] + (float_t(1) - momentum_) * variance_current_[i];
    }
  }
