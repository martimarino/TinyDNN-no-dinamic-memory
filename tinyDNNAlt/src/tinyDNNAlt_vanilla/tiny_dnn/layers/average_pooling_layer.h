  ...
  for_i(parallelize, in_data[0]->size(), [&](size_t sample) {
    ...
    for (size_t d = 0; d < out_dim.depth_; ++d) {
      ...
      for (size_t i = 0; i < oarea; ++i, ++idx) {
        ...
        float_accum_t value{0};
        for (auto connection : connections) value += float_accum_t(in[connection.second]);
        value *= float_accum_t(weight);
        value += float_accum_t(bias);
        out[idx] = float_t(value);
      }
    }
 ...
 public:
  ...
  average_pooling_layer(size_t in_width,
    ...
    : Base(
        ...
        in_channels,
        in_channels,
        float_t(1) / float_t((double(pool_size_x * pool_size_y)))), //WORKAROUND__B ____E
      stride_x_(stride_x),
      stride_y_(stride_y),
 ...
 private:
  ...
  static size_t pool_out_dim(size_t in_size,
    ...
    return static_cast<int>(
      std::ceil((static_cast<double>(in_size) - pooling_size) / stride) + 1); //WORKAROUND__B
  }
