...
class layer : public node {
 public:
   ...
   *
   **/
  layer(const etl::vector<vector_type, MAX_CHANNEL_SIZE> &in_type,
        const etl::vector<vector_type, MAX_CHANNEL_SIZE> &out_type)
  ...
  etl::vector<shape3d, MAX_CHANNEL_SIZE> in_data_shape() {  // shape for each input data
    return filter(in_shape(), [&](size_t i) {  // NOLINT
      return in_type_[i] == vector_type::data;
    }, MAX_CHANNEL_SIZE);
  }

  etl::vector<shape3d, MAX_CHANNEL_SIZE> out_data_shape() { // shape for each output data
  ...
  void output(etl::vector<const tensor_t *, 1> &out) const {
    out.clear();
    for (size_t i = 0; i < out_channels_; i++) {
  ...
  etl::vector<vector_type, MAX_CHANNEL_SIZE> in_types() const { return in_type_; }

  etl::vector<vector_type, MAX_CHANNEL_SIZE> out_types() const { return out_type_; }
   ...
   **/
  virtual etl::vector<shape3d, MAX_CHANNEL_SIZE> in_shape() const = 0;
   ...
   **/
  virtual etl::vector<shape3d, MAX_CHANNEL_SIZE> out_shape() const = 0;
...
/// visualization.
   ...
   * class integrated?
   */
  void forward(const etl::vector<tensor_t, MAX_CHANNEL_SIZE> &input,
               etl::vector<const tensor_t *, 1> &out) {  // for test
    ...
    for (size_t i = 0; i < in_type_.size(); i++) {
      if (trainable() && is_trainable_weight(in_type_[i])) {
        ...
        float_t rcp_batch_size =
          float_t(1.0) / float_t(double(ith_in_node(i)->get_data()->size()));
		//WORKAROUND__E
 ...
 protected:
  ...
  etl::vector<vector_type, MAX_CHANNEL_SIZE> in_type_;
  /** Vector containing the type of data for outputs */
  etl::vector<vector_type, MAX_CHANNEL_SIZE> out_type_;
 ...
 private:
  ...
  etl::vector<tensor_t *, MAX_CHANNEL_SIZE> fwd_out_data_;
  etl::vector<tensor_t *, MAX_CHANNEL_SIZE> bwd_in_data_;
  etl::vector<tensor_t *, MAX_CHANNEL_SIZE> bwd_in_grad_;
  etl::vector<tensor_t *, MAX_CHANNEL_SIZE> bwd_out_data_;
  etl::vector<tensor_t *, MAX_CHANNEL_SIZE> bwd_out_grad_;
...
std::ostream &operator<<(std::ostream &s, const etl::vector<shape3d, MAX_CHANNEL_SIZE> &vec) {
