...
class lrn_layer : public layer {
 public:
  lrn_layer(const shape3d &in_shape,
            size_t local_size,
            float_t alpha      = float_t(1.0),
            float_t beta       = float_t(5.0),
 ...
 private:
  void forward_across(const vec_t &in, vec_t &out) {
    ...
    const float_t alpha_div_size = alpha_ / float_t(double(size_));
    ...
    for (size_t i = 0; i < channels; i++, head++, tail++) {
      ...
      for (size_t j = 0; j < wxh; j++)
        dst[j]      = src[j] *
                 tiny_dnn::pow(float_t(1) + alpha_div_size * in_square_[j], -beta_);
    }
  }
