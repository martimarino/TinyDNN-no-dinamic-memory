...
namespace tiny_dnn {
    ...
    void construct_graph(network<graph>& graph,
                         const etl::vector<layer*, MAX_INPUT_SIZE>& inputs,
                         const etl::vector<layer*, MAX_OUTPUT_SIZE>& outputs);
 ...
 **/
    ...
    class network {
    public:
        typedef typename etl::vector<layer *, MAX_LAYERS>::iterator iterator;
        typedef typename etl::vector<layer *, MAX_LAYERS>::const_iterator const_iterator;
        NetType net_;
         ...
         * executes forward-propagation and returns output
         *
         * @param in input value range(double[], etl::vector<double>,
         *std::list<double> etc)
         **/
        ...
        bool train(Optimizer &optimizer,
                   ...
                   const etl::vector<label_t, MAX_INPUT_SIZE> &class_labels,
                   size_t batch_size,
                   int epoch,
                   ...
                   const etl::vector<vec_t, MAX_INPUT_SIZE> &t_cost = etl::vector<vec_t, MAX_INPUT_SIZE>()) {
	 ...
	 * network<sequential> net;
        ...
        bool fit(Optimizer &optimizer,
                 const etl::vector<T, MAX_INPUT_SIZE> &inputs,
                 const etl::vector<U, MAX_INPUT_SIZE> &desired_outputs,
                 size_t batch_size,
                 int epoch,
                 OnBatchEnumerate on_batch_enumerate,
                 OnEpochEnumerate on_epoch_enumerate,
        ...
        template <typename Error, typename Optimizer>
        bool train(Optimizer &optimizer,
                   const etl::vector<vec_t, MAX_INPUT_SIZE> &in,
                   const etl::vector<vec_t, MAX_INPUT_SIZE> &t,
                   size_t batch_size = 1,
                   int epoch         = 1) {
         ...
         **/
        etl::vector<vec_t, MAX_INPUT_SIZE> test(const etl::vector<vec_t, MAX_INPUT_SIZE> &in) {
            etl::vector<vec_t, MAX_INPUT_SIZE> test_result(in.size());
            set_netphase(net_phase::test);
            for (size_t i = 0; i < in.size(); i++) {
         ...
         **/
        template <typename E>
        float_t get_loss(const etl::vector<vec_t, MAX_INPUT_SIZE> &in,
         ...
         **/
        template <typename E>
        float_t get_loss(const etl::vector<vec_t, MAX_INPUT_SIZE> &in, const etl::vector<vec_t, MAX_INPUT_SIZE> &t) {
            float_t sum_loss = float_t(0);
        ...
        void fast_load(const char *filepath) {
            ...
            etl::vector<float_t, MAX_TENSOR_SIZE> data;
            double temp;
    ...
    private:
        ...
        friend network<sequential> &operator<<(network<sequential> &n, etl::vector<layer*, MAX_LAYERS> &&ls);

        friend void construct_graph(
                network<graph> &graph,
                const etl::vector<std::shared_ptr<layer>, MAX_INPUT_SIZE> &inputs,
                const etl::vector<std::shared_ptr<layer>, MAX_INPUT_SIZE> &outputs);

        friend void construct_graph(network<graph> &graph,
                                    const etl::vector<layer *, MAX_INPUT_SIZE> &inputs,
                                    const etl::vector<layer *, MAX_INPUT_SIZE> &outputs);
        ...
        bool calc_delta(const etl::vector<tensor_t, MAX_INPUT_SIZE> &in,
            ...
            static const float_t delta =
                    tiny_dnn::sqrt(std::numeric_limits<float_t>::epsilon());
            ...
            return std::abs(delta_by_bprop - delta_by_numerical) <= float_t(eps);
        }
        ...
        template <typename T>
        void check_training_data(const etl::vector<vec_t, MAX_INPUT_SIZE> &in,
                                 const etl::vector<T, MAX_INPUT_SIZE> &t) {
 ...
 */
    inline etl::vector<vec_t, MAX_TENSOR_SIZE> image2vec(const float_t *data,
                                        const size_t rows,
                                        const size_t cols,
                                        const size_t sizepatch,
                                        const size_t step = 1) {
        assert(step > 0);
        etl::vector<vec_t, MAX_TENSOR_SIZE> res(
    ...
    network<sequential> &operator<<(network<sequential> &n, etl::vector<layer*, MAX_LAYERS> &&ls) {
        for(auto l:ls)
            n.net_.add(*l);
        return n;
    }
