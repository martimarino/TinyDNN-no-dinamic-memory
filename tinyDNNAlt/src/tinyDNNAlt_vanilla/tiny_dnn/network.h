 ...
 **/
    ...
    public:
         ...
         * executes forward-propagation and returns output
         **/
        std::vector<tensor_t> predict(const etl::vector<tensor_t, MAX_INPUT_SIZE> &in) {
            return fprop(in);
        }
    ...
    private:
        ...
        friend network<sequential> &operator<<(network<sequential> &n, std::vector<layer*> &&ls);
 ...
 */
    ...
    network<sequential> &operator<<(network<sequential> &n, std::vector<layer*> &&ls) {
        for(auto l:ls)
            n.net_.add(*l);
        return n;
    }
