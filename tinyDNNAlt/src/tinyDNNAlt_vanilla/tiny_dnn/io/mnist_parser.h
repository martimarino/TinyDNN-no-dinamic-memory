...
inline void parse_mnist_image(std::ifstream &ifs,
  ...
  dst.resize(width * height, float_t(scale_min));

  for (uint32_t y = 0; y < header.num_rows; y++)
    for (uint32_t x = 0; x < header.num_cols; x++) {
      double val =  ((double(image_vec[y * header.num_cols + x])) / double(255)) *(scale_max - scale_min) +
        scale_min;
      dst[width * (y + y_padding) + x + x_padding] = float_t(val);
      assert(val == double(dst[width * (y + y_padding) + x + x_padding]));
    }
}
