...
template <size_t width,size_t height,size_t depth,class LabelType> 
inline void parse_binImagenet_features(const std::string &filename,
                          std::vector<vec_t> *train_images,
                          std::vector<label_t> *train_labels,
                          float_t scale_min,
                          float_t scale_max) {
  ...
  if (ifs.fail() || ifs.bad())
    throw nn_error("failed to open file:" + filename);
  LabelType label;
  std::vector<float> buf(width*height*depth);

  while (ifs.read(reinterpret_cast<char *>(&label), sizeof(LabelType))) {
    vec_t img;
    if (!ifs.read(reinterpret_cast<char *>(&buf[0]), 4*width*height*depth)) break;
      std::transform(buf.begin(), buf.end(), std::back_inserter(img),
                     [=](float f) {
                       return (tiny_dnn::float_t)f;
                     });
    train_images->push_back(img);
    train_labels->push_back(label);
  }
