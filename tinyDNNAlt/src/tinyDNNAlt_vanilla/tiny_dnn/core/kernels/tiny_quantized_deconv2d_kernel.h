...
inline void tiny_quantized_deconv2d_kernel(const deconv_params &params,
  ...
  if (min_filter == max_filter) {
    max_filter = W[0] + float_t(1e-3f);
    min_filter = W[0] - float_t(1e-3f);
  }
  ...
  if (params.has_bias) {
    ...
    if (min_bias == max_bias) {
      max_bias = bias[0] + float_t(1e-3f);
      min_bias = bias[0] - float_t(1e-3f);
    }
    bias_quantized =
  ...
  const int32_t offset_input = int64_to_int32(
    float_to_quantized_unclamped<uint8_t>(float_t(0.0f), min_input, max_input));
  const int32_t offset_filter = int64_to_int32(
    float_to_quantized_unclamped<uint8_t>(float_t(0.0f), min_filter, max_filter));
  const int32_t zero_in_total_space = int64_to_int32(
    float_to_quantized<int32_t>(float_t(0.0f), min_output_value, max_output_value));
...
inline void tiny_quantized_deconv2d_back_kernel(const deconv_params &params,
  ...
  const int32_t offset_prev_out = int64_to_int32(
    float_to_quantized_unclamped<uint8_t>(float_t(0.0f), min_prev_out, max_prev_out));
  const int32_t offset_filter = int64_to_int32(
    float_to_quantized_unclamped<uint8_t>(float_t(0.0f), min_filter, max_filter));
  const int32_t offset_curr_delta =
    int64_to_int32(float_to_quantized_unclamped<uint8_t>(float_t(0.0f), min_curr_delta,
                                                         max_curr_delta));
...
inline void tiny_quantized_deconv2d_kernel(const deconv_params &params,
  ...
  if (W_r[0] == W_r[1]) {
    max_filter = W_r[1] + float_t(1e-3f);
    min_filter = W_r[0] - float_t(1e-3f);
  }
  // output range
  float_t min_output_value;
  float_t max_output_value;
  ...
  for (size_t i = 0; i < in.size(); i++) {
    in_quantized.push_back((double)static_cast<uint8_t>(double(in[i])));
  }
  for (size_t i = 0; i < W.size(); i++) {
    W_quantized.push_back((double)static_cast<uint8_t>((double)W[i]));
  }
  for (size_t i = 0; i < bias.size(); i++) {
    bias_quantized.push_back((double)static_cast<uint8_t>((double)bias[i]));
  }
  ...
  // calculating offset
  const int32_t offset_input = int64_to_int32(
    float_to_quantized_unclamped<uint8_t>(float_t(0.0f), in_r[0], in_r[1]));
  const int32_t offset_filter = int64_to_int32(
    float_to_quantized_unclamped<uint8_t>(float_t(0.0f), min_filter, max_filter));
  const int32_t zero_in_total_space = int64_to_int32(
    float_to_quantized<int32_t>(float_t(0.0f), min_output_value, max_output_value));
  ...
  for_i(layer_parallelize, params.out.depth_, [&](size_t o) {
    ...
    if (params.has_bias) {
      ...
      std::for_each(pout_quantized, poutout_quantized, [&](int32_t &f) {
        f += static_cast<int32_t>((bias[o] - float_t(double(zero_in_total_space))));
      });
    }
  });
