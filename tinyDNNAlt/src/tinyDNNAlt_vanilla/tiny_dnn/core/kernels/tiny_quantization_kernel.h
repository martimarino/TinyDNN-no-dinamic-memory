...
int64_t float_to_quantized_unclamped(float_t input,
  ...
  const double range            = (range_max - range_min) * float_t(range_adjust);
  const double range_scale      = (number_of_steps / range);
  int64_t quantized = static_cast<int64_t>(round(double(input) * range_scale) -
                                           round(double(range_min) * range_scale));
...
float quantized_to_float(T input, float_t range_min, float_t range_max) {
  ...
  const double result       = range_min + float_t((offset_input * range_scale));
  return float(result);
}
...
float float_for_one_quantized_level(float_t range_min, float_t range_max) {
  ...
  const float float_for_one_quantized_level =
    (range_max - range_min) / float_t(double((highest_ - lowest_)));
...
void quantization_range_for_multiplication(float_t min_a,
  ...
  const float_t a_float_for_one_quant_level =
    float_t(float_for_one_quantized_level<T1>(float_t(min_a), float_t(max_a)));
  const float_t b_float_for_one_quant_level =
    float_t(float_for_one_quantized_level<T2>(float_t(min_b), float_t(max_b)));
...
inline void requantize_many_in_new_range<int32_t, uint8_t>(int32_t *input,
  ...
  const float input_range        = float(max_input - min_input);
  const float output_range       = float(max_output - min_output);
  ...
  const int64_t input_offset_fp = static_cast<int64_t>(double(
    (min_input * float_t(double(recip_output_range_fp))) + float_t(double((range_scale_fp >> 1)))));
  const int64_t output_offset_fp =
    static_cast<int64_t>(round(double((min_output * float_t(255.0f)) / float_t(double(output_range)))));
