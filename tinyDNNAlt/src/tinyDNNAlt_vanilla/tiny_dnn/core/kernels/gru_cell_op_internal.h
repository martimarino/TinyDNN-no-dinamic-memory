       ...
       [&](const blocked_range &range) {
         ...
         for (size_t sample = range.begin(); sample < range.end(); sample++) {
           ...
           for (size_t o = 0; o < out_size; o++) {
             float_t local_z = float_t(0.0);
             float_t local_r = float_t(0.0);
             float_t local_h = float_t(0.0);
  ...
  for_(
    ...
    [&](const blocked_range &range) {
      ...
      for (size_t sample = range.begin(); sample < range.end(); sample++) {
        ...
        vec_t aux1(out_size, float_t(0.0));
        vec_t aux2(out_size, float_t(0.0));
