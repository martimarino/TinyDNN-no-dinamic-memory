...
class xavier : public scalable {
 public:
  ...
  void fill(vec_t *weight, size_t fan_in, size_t fan_out) override {
	//WORKAROUND__BEGIN
	  const float_t weight_base = tiny_dnn::sqrt(scale_ / float_t(double(fan_in + fan_out)));
	//const float_t weight_base = tiny_dnn::sqrt(scale_ / (fan_in + fan_out));
	//WORKAROUND__END
...
class lecun : public scalable {
 public:
  ...
  void fill(vec_t *weight, size_t fan_in, size_t fan_out) override {
	...
	//WORKAROUND__BEGIN
	const float_t weight_base = scale_ / tiny_dnn::sqrt(float_t(double(fan_in)));
...
class he : public scalable {
 public:
  ...
  void fill(vec_t *weight, size_t fan_in, size_t fan_out) override {
	...
	//WORKAROUND__BEGIN
	const float_t sigma = tiny_dnn::sqrt(scale_ / float_t(double(fan_in)));
