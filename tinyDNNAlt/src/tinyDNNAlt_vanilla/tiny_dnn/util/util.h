...
#endif
namespace tiny_dnn {
    float_t sqrt(float_t a) {
        double b(a);
        return float_t(std::sqrt(b));
    }

    float_t log(float_t a) {
        double b(a);
        return float_t(std::log(b));
    }

    float_t asinh(float_t a) {
        double b(a);
        return float_t(std::asinh(b));
    }

    float_t tanh(float_t a) {
        double b(a);
        return float_t(std::tanh(b));
    }

    float_t pow(float_t b, float_t e) {
        double db(b), de(e);
        return float_t(std::pow(db, de));
    }


    float_t cosh(float_t a) {
        double b(a);
        return float_t(std::cosh(b));
    }

    float_t exp(float_t a) {
        double b(a);
        return float_t(std::exp(b));
    }

    float_t log1p(float_t a) {
        double b(a);
        return float_t(std::log1p(b));
    }
//          y[j] = float_t(1) / (float_t(1) + tiny_dnn::exp(-x[j]));

    float_t sigmoid(float_t x) {
#ifdef CNN_USE_POSIT
        #if(CNN_EXP_BITS == 0)
        return x.pseudosigmoid();
      #else 
       return float_t(1) / (float_t(1) + tiny_dnn::exp(-x));
      #endif
#else
        return float_t(1) / (float_t(1) + tiny_dnn::exp(-x));
#endif
...
/// of classes
    ...
    typedef etl::vector<vec_t, N> tensor_t;
    ...
    template <typename U>
    U rescale(float_t x, float_t src_min, float_t src_max, U dst_min, U dst_max) {
        U value = static_cast<U>(
                ((x - src_min) * float_t(double((dst_max - dst_min)))) / (src_max - src_min) + float_t(double(dst_min)));
