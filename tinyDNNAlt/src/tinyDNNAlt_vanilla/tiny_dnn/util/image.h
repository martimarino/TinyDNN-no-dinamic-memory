...
image<float_t> mean_image(const image<T> &src) {
  ...
  for (size_t i = 0; i < src.depth(); i++) {
    ...
    mean.at(0, 0, i) = sum / float_t(double((src.width() * src.height())));
  }
...
/**
 * convert from vector with [oMin,oMax] intensities to [nMin,nMax] image
 *
 */

template<typename T>
image<T> fromNormalizedVec(vec_t& vec,int width,int height,image_type type,T oMin,T oMax,T nMin,T nMax) {
  size_t depth = (type==image_type::grayscale)?1:3;
  vec_t unNorm = vec_t(vec);
  for(auto it = unNorm.begin(); it != unNorm.end(); ++it)
    *it = ((*it)-oMin)/(oMax-oMin)*(nMax-nMin)+nMin;
  image<T> img = image<T>(&unNorm[0],width,height,type);
  return img;
}

template<typename T>
vec_t resizeToVec(image<T> img,int nWidth,int nHeight) {
    return resize_image(img,nWidth,nHeight).to_vec();
}
/**
 * visualize 1d-vector
 *
