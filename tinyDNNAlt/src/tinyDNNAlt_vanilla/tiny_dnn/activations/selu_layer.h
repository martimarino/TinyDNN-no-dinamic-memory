...
class selu_layer : public activation_layer {
 public:
  ...
  explicit selu_layer(const float_t lambda = float_t(1.05070),
                      const float_t alpha  = float_t(1.67326))
  ...
  selu_layer(size_t in_dim,
             const float_t lambda = float_t(1.05070),
             const float_t alpha  = float_t(1.67326))
  ...
  void forward_activation(const vec_t &x, vec_t &y) override {
    for (size_t j = 0; j < x.size(); j++) {
      y[j] =
        lambda_ *
        (x[j] > float_t(0) ? x[j] : alpha_ * (tiny_dnn::exp(x[j]) - float_t(1)));
    }
  }
  ...
  void backward_activation(const vec_t &x,
    ...
    for (size_t j = 0; j < x.size(); j++) {
      dx[j] =
        dy[j] * lambda_ * (x[j] > float_t(0) ? float_t(1.0) : alpha_ * tiny_dnn::exp(x[j]));
    }
  }
