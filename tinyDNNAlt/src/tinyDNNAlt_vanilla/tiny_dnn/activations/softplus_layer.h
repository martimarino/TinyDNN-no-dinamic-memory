...
class softplus_layer : public activation_layer {
 public:
   ...
   */
  explicit softplus_layer(const float_t beta      = float_t(1.0),
                          const float_t threshold = float_t(20.0))
  ...
  softplus_layer(size_t in_dim,
                 const float_t beta      = float_t(1.0),
                 const float_t threshold = float_t(20.0))
  ...
  void forward_activation(const vec_t &x, vec_t &y) override {
    for (size_t j = 0; j < x.size(); j++) {
      ...
      y[j]           = (betain > threshold_) ? x[j]
                                   : (float_t(1) / beta_) * tiny_dnn::log1p(tiny_dnn::exp(betain));
    }
  }
  ...
  void backward_activation(const vec_t &x,
    ...
    for (size_t j = 0; j < x.size(); j++) {
      ...
      float_t exp_bo  = tiny_dnn::exp(betaout);
      // dx = dy * (gradient of softplus)
      dx[j] = (betaout > threshold_) ? dy[j] : dy[j] * (exp_bo - float_t(1)) / exp_bo;
    }
  }
