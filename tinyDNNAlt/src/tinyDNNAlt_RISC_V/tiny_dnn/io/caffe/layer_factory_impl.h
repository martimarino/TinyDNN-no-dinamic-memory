...
inline std::shared_ptr<layer> create_flatten(const caffe::LayerParameter &layer,
                                           const shape_t &bottom_shape,
                                           shape_t *top_shape) {
  auto flatten = std::make_shared<flatten_layer>(bottom_shape.width_,bottom_shape.height_,bottom_shape.depth_);
  *top_shape = flatten->out_shape()[0];
  return flatten;
}
  ...
  *top_shape = bottom_shape;
  ...
  if (layer.blobs_size() > 0) {
    ...
    for (size_t i = 0; i < mean.size(); i++) {
      mean[i]     = float_t(global_stats.Get(0).data(i)) * scale_factor;
      variance[i] = float_t(global_stats.Get(1).data(i)) * scale_factor;
    }
    bn->set_mean(mean);
...
inline bool layer_skipped(const std::string &type) {
  if (type == "Data" || type == "EuclideanLoss" || type == "Input" ||
      type == "HDF5Data" || type == "Split" || type == "Accuracy" || type == "Scale")
    return true;
  return false;
}
...
inline std::shared_ptr<layer> create(const caffe::LayerParameter &layer,
  ...
  if (layer_type == "Flatten") {
    return detail::create_flatten(layer, in_shape, out_shape);
  }
