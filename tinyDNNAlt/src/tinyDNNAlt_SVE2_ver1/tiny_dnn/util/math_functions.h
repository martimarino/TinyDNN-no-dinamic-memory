    ...
    for (size_t j = 0; j < channels; j++) {
      ...
      rvar             = std::accumulate(it, it + spatial_dim, rvar,
                             [ex](float_t current, float_t x) {
								return current + (x - ex)*(x - ex); /*WORKAROUND__B pow(x - ex, float_t{2.0});*/
                             });
    }
  }
  vector_div(
    ...
    std::max(float_t{1.0f},
             float_t(static_cast<double>(num_examples * spatial_dim)) - float_t{1.0f})); //WORKAROUND__B
}
...
inline void moments(const tensor_t &in,
  ...
  vector_div(mean, (float_t)double(num_examples) * float_t(double(spatial_dim))); //WORKAROUND__B
}
