...
struct scalar_generic {
  ...
  static CNN_MUST_INLINE register_type mul(const register_type &v1,
    ...
    assert(double(v1)*double(v2)==double(v1*v2));
    return v1 * v2;
  }
  ...
  static CNN_MUST_INLINE register_type madd(const register_type &v1,
    ...
    assert(double(v1)*double(v2)+double(v3)==double(v1*v2+v3));
    return v1 * v2 + v3;
  }
...
CNN_MUST_INLINE typename T::value_type dot_product(
  ...
  std::size_t size) {
//  std::cout << "dot_product called!!" << std::endl;
...
#else
template <typename T, typename f1_aligned, typename f2_aligned>
CNN_MUST_INLINE tiny_dnn::float_t dot_product(const tiny_dnn::float_t* f1,const tiny_dnn::float_t* f2,std::size_t size) {
  //std::cout << "Using custom dot product with float accumulator" << std::endl;
  return tiny_dnn::float_t::dot<const tiny_dnn::float_t*,const tiny_dnn::float_t*>(f1,f2,size,1.0f);
}
#endif
